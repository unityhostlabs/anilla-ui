...
function configure(overrides = {}) {
	const invalid = Object.keys(overrides).filter((k) => !(k in defaults$2));
...
function resetConfig() {
	Object.assign(config, defaults$2);
	return config;
}
/**
* Internal logger
	...
	*/
	clear(keyPrefix) {
		if (!isEmpty(keyPrefix)) {
			for (const key in this.#storage) if (key.startsWith(`${this.#prefix}${this.#delimiter}`)) this.#storage.removeItem(key);
			return;
		}
		this.#storage.clear();
	}
...
var Theme = class extends BaseComponent {
	...
	_init() {
		...
		const _onTrigger = (e) => {
			...
			if (this._isClickable(trigger)) setAttributes(trigger, {
				ariaPressed: this._getTheme() === this._modes.dark,
				ariaCurrent: this._getTheme() === this._modes.dark
			});
		};
		if (this._isClickable(this.el)) this.addListener(this.el, "click", _onTrigger);
		if (this._isChangeable(this.el)) if (this.el.type === "radio") this._getRadioGroup().forEach((radio) => {
			this.addListener(radio, "change", _onTrigger);
		});
		else this.addListener(this.el, "change", _onTrigger);
		...
		if (this._getTheme() === this._modes.auto && window.matchMedia("(prefers-color-scheme: dark)").matches) addClasses(this.options.parent, this.options.className);
	}
	/** @returns {string} */
	_getTheme() {
	...
	*/
	_getMode(el) {
		if (el.type === "checkbox" && this.options.toggle) return el.checked ? this._modes.dark : this._modes.light;
		if (this._isClickable(el) && this.options.toggle) return getAttribute(el, "aria-pressed") === "true" ? this._modes.dark : this._modes.light;
	...
	*/
	change(mode) {
		...
		if (mode === this._modes.auto) this.storage.remove(this._storageKey);
		else this.storage.set(this._storageKey, mode);
...
//#endregion
export { AutoInit, BaseComponent, DOMEventStore, DataStorage, EventBus, Registry, Theme, Transition, bootAutoInit, config, configure, logger, resetConfig, utils_exports as utils };
